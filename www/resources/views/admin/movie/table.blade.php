                        ...
                        <div class="dropdown-menu">
                            @can('movie.edit')
                                <a class="dropdown-item" onclick="showMovieEditModel(event)"
                                    href="{{ route('movie.edit', $row->id) }}">Edit</a>
                            @endcan

                            @if ($row->is_active == 'Y')
                                <a class="dropdown-item" href="{{ route('movie.status', $row->id) }}">Inactive</a>
                            @else
                                <a class="dropdown-item" href="{{ route('movie.status', $row->id) }}">Active</a>
                            @endif
                            @can('movie.destroy')
                                <a class="dropdown-item"
                                    onclick="if(confirm('Are you sure you want to delete.')) document.getElementById('delete-{{ $row->id }}').submit()">
                                    Delete</a>
                                <form id="delete-{{ $row->id }}" action="{{ route('movie.destroy', $row->id) }}"
                                    method="POST">
                                    @method('DELETE')
                                    @csrf
                                </form>
